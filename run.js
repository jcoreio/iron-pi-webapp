...
for (let coverage of [false, true]) {
  ...
  for (let watch of coverage ? [false] : [false, true]) {
    for (let debug of watch ? [false] : [false, true]) {
      ...
      task(`${prefix}${suffix}`, ['node_modules', seleniumServer, services], testRecipe({unit: true, selenium: true, integration: true, coverage, watch, debug}))
      ...
      task(`${prefix}:unit${suffix}`, ['node_modules'], testRecipe({unit: true, coverage, watch, debug}))
        .description(`run integration tests${coverage ? ' with code coverage' : ''}${watch ? ' in watch mode' : ''}${debug ? ' in debug mode' : ''}`)
      task(`${prefix}:integration${suffix}`, ['node_modules', services], testRecipe({integration: true, coverage, watch, debug}))
